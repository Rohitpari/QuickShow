  ...
  const fetchNowplayingMovies = async () => {
    ...
    const handleSubmit = async()=>{
      try {
        ...
        console.log("datetime",dateTimeSelection);
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
        ...
        <div className='group flex flex-wrap gap-4 mt-4 w-max'>
          ...
          group-hover:not hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={() => setSelectedMovies(movie.id)}>
              ...
              {SelectedMovies === movie.id && (
                <div className='absolute top-2 right-2 fex items-center justify-center bg-primary h-6 w-6 rounded'>
                  <CheckIcon className='w-4 h-4 text-white' strokeWidth={2.5} />
                </div>
              )}
              <p className='font-medium truncate'>{movie.title}</p>
              <p className='font-gray-400 text-sm'>{movie.release_date}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className='mt-8'>
        ...
        <div className='inline-flex items-center gap-2 border-gray-600 px-3 py-2 rounded-md'>
